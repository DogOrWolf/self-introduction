...
https://shimo.im/docs/6VdchtKxdjrCqDdY
...
1. 面对众多的 Go 语言项目包依赖管理工具无所适从，不知道该用哪一个工具
...
## **模块****七****：****Kubernetes** **控制平面组件****：调度器和控制器**
...
1. 全方位的 CSI 一站式解决方案：Rook



## **模块****八****：**Kubernetes 控制平面组件：生命周期管理和服务发现
...
1. 基于 Kubernetes、Jekins、Tekton 打造 CI/CD Pipeline

1. 集群监控：日志收集和分析，事件、指标和告警（Event、Metrics 和 Alert）

## **模块****十一****：**将应用迁移至 Kubernetes 平台

### 教学目标：

1. 如何将无状态应用发布到 Kubernetes 平台

1. 如何基于 Kubernetes 原生能力将有状态应用发布到 Kubernetes 平台
...
## **模块****十三****：**Kubernetes 集群联邦和 Istio 多集群管理
